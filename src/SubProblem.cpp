...
int spp_spptw_res_cont::initMinDays = 0;
...
bool operator==( const spp_spptw_res_cont_Short& res_cont_1, const spp_spptw_res_cont_Short& res_cont_2 ){
...
// 1 cost + 2 time resource comparisons (== and <)
bool operator==( const spp_spptw_res_cont& res_cont_1, const spp_spptw_res_cont& res_cont_2 ){
	return ( res_cont_1.cost == res_cont_2.cost and res_cont_1.time[MAX_DAYS] == res_cont_2.time[MAX_DAYS]
		 and res_cont_1.time[MIN_DAYS] == res_cont_2.time[MIN_DAYS]);
}
...
bool operator<( const spp_spptw_res_cont& res_cont_1, const spp_spptw_res_cont& res_cont_2 ){
  ...
  if( res_cont_1.cost > res_cont_2.cost )
    ret = false;
  else if( res_cont_1.cost == res_cont_2.cost) {
    if ( res_cont_1.time[MAX_DAYS] > res_cont_2.time[MAX_DAYS] )
      ret = false;
    else if ( res_cont_1.time[MAX_DAYS] == res_cont_2.time[MAX_DAYS] and res_cont_1.time[MIN_DAYS] >= res_cont_2.time[MIN_DAYS] )
      ret = false;
  }
...
SubProblem::SubProblem(Scenario * scenario, int nbDays, const Contract * contract, vector<State>* pInitState, bool noShort):
  ...
  spp_spptw_res_cont::initMinDays = CDMin_;
...
Rotation SubProblem::rotationFromPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_spptw_res_cont resource){
	...
	for( int j = static_cast<int>( path.size() ) - 1; j >= 0;	--j){
		...
		// A. Arc from source (equivalent to short rotation)       /////////// SERGEB A MODIFIER
		if(aType == SOURCE_TO_PRINCIPAL){
		  firstDay =  principalToDay_[destin] - daysMin_ + 1;
...
void SubProblem::createArcsSourceToPrincipal(){
  ...
  for(int sh=1; sh<nShiftsType; sh++){
	...
	for (int s=0; s < nb; s++) {
	  ...
	  addSingleArc(origin, destin, 0, daysMin_, SOURCE_TO_PRINCIPAL, shiftID);
	}
				
	//     arcsFromSource_[sh][k][nCons] = nArcs_;
	//     addSingleArc(origin, destin, 0, daysMin_, SOURCE_TO_PRINCIPAL, origin);
	///////  SERGEB:    plusieurs arcs Orig-Dest (1 par shift)
      }
    }
  }
}
		...
		for(int k=daysMin_-1; k<nDays_-1; k++){
			...
			for(int nCons=1; nCons<maxvalConsByShift_[sh]; nCons ++){
				...
				for (unsigned int s = 0; s < nShifts; s++) {
				  ...
				  addSingleArc(origin, destin, 0, 1, SHIFT_TO_SAMESHIFT, shiftID);
				}
			}
			...
			// addSingleArc(origin, destin, cost, 1, REPEATSHIFT);
			  ...
			  addSingleArc(origin, destin, cost, 1, REPEATSHIFT, shiftID);
			}
			...
			for(int newSh=1; newSh<nShiftsType; newSh++){
			  if(newSh != sh and ! pScenario_->isForbiddenSuccessorShiftType_ShiftType(newSh,sh)){
			    ...
			    for (unsigned int s = 0; s < nNewShifts; s++) {
			      ...
			      addSingleArc(origin, destin, 0, 1, SHIFT_TO_NEWSHIFT, newShiftID);
			    }
			  }
			}
...
void SubProblem::updateArcCosts(){
  ...
  //	A. ARCS : SOURCE_TO_PRINCIPAL [baseCost = 0]    //////////  SERGEB A MODIFIER
  ...
  for(int sh=1; sh<pScenario_->nbShiftsType_; sh++){
	...
	for (int s=0; s < pScenario_->shiftTypeIDToShiftID_[sh].size(); s++) {
	  ...
	  int shiftID = pScenario_-> shiftTypeIDToShiftID_[sh][s];
	  //	  double c = arcCostBestShortSuccCDMin_[sh][k][n][s];
	  
	  //	  double c = costArcSource(a, k, s);

	  vector <int> succ;
	  succ.push_back(shiftID);
	  double c = costOfVeryShortRotation(k, succ);	  
	  updateCost( a , c );
	  //	  shortSuccCDMinIdFromArc_.insert(pair<int,int>( arcsFromSource_[sh][k][n][s], idBestShortSuccCDMin_[sh][k][n]));
	}
      }
    }
  }
  ...
  for(int sh=1; sh<pScenario_->nbShiftsType_; sh++){
	...
	for (int s=0; s < pScenario_->shiftTypeIDToShiftID_[sh].size(); s++) {
	  ...
	  updateInitialTime(a, daysMin_ + pLiveNurse_->pStateIni_->consDaysWorked_);
	  //	  updateTime(a, pScenario_->hoursToWork_[shiftID] + pLiveNurse_->pStateIni_->totalTimeWorked_ );   /////////    SERGEB :  1 arc par shift    + hoursWorked
	}
    }
  }
...
int SubProblem::normalTravelTime(int a){     // ??????  SERGEB
  ...
  return (getInitialTime(a));
	...
	// 	  return (daysMin_ + pLiveNurse_->pStateIni_->totalTimeWorked_);    /////  SERGEB : hoursWorked
	// 	} else {
	// 		return daysMin_;
	// 	}
	...
	// 	// }
	// }
	// // else if(atype == SHIFT_TO_NEWSHIFT or atype == SHIFT_TO_SAMESHIFT or atype == REPEATSHIFT) return 1;    /////  SERGEB : hoursWorked
	// else return 0;
}
...
void SubProblem::printPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_spptw_res_cont resource){
	...
	for( int j = static_cast<int>( path.size() ) - 1; j >= 0;	--j){
		...
		int destin = boost::target( path[j], g_ );
		if(origin == sourceNode_){                                    //////  SERGEB A MODIFIER
			int firstDay =  principalToDay_[destin] + 1;
			while(k<firstDay){
				std::cout << " |";
				k++;
			}
...
SubProblemShort::SubProblemShort(Scenario * scenario, int nbDays, const Contract * contract, vector<State>* pInitState, bool noShort):
  SubProblem(scenario, nbDays, contract,  pInitState, noShort) {
...
// For the long rotations, depends wether we want optimality or not
bool SubProblemShort::solveLongRotations(bool optimality){
	if(optimality){
		updateArcCosts();						// Update costs
		return solveLongRotationsOptimal();		// Solve shortest path problem
	}
...
// Function called when optimal=true in the arguments of solve -> shortest path problem is to be solved
bool SubProblemShort::solveLongRotationsOptimal(){

	vector< vector< boost::graph_traits<Graph>::edge_descriptor> > opt_solutions_spptw;
	vector<spp_spptw_res_cont_Short> pareto_opt_rcs_spptw;
	...
	if(!(param_.oneSinkNodePerLastDay_)){
		r_c_shortest_paths(
				...
				opt_solutions_spptw,
				pareto_opt_rcs_spptw,
				spp_spptw_res_cont_Short (0,0),
				ref_spptw_Short(),
				dominance_spptw_Short(),
				std::allocator< boost::r_c_shortest_paths_label< Graph, spp_spptw_res_cont_Short> >(),
				boost::default_r_c_shortest_paths_visitor() );
		return addRotationsFromPaths(opt_solutions_spptw, pareto_opt_rcs_spptw);
	}
...
// Transforms the solutions found into proper rotations.
//
bool SubProblemShort::addRotationsFromPaths(vector< vector< boost::graph_traits<Graph>::edge_descriptor> > paths, vector<spp_spptw_res_cont_Short> resources){
	int nFound = 0;
	// For each path of the list, record the corresponding rotation (if negativeOnly=true, do it only if the dualCost < 0)
	for(unsigned int p=0; p < paths.size(); ++p){
		...
		bool b_feasible = false;
		bool b_correctly_extended = false;
		spp_spptw_res_cont_Short actual_final_resource_levels( 0, 0 );
		boost::graph_traits<Graph>::edge_descriptor ed_last_extended_arc;
		check_r_c_path( g_,
				paths[p],
				spp_spptw_res_cont_Short( 0, 0 ),
...
// Adds a rotation made from the given path to the current list of answers and increases their counter
//
Rotation SubProblemShort::rotationFromPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_spptw_res_cont_Short resource){
	...
	for(int s=1; s<pScenario_->nbShiftsType_; s++){
		for(int k=daysMin_-1; k<nDays_; k++){
			for(int n=1; n<=maxvalConsByShift_[s]; n++){
				...
				// IF NO VALID SUCCESSION OR IF THE FIRST DAY IS FORBIDDEN AS A STARTING DAY, THEN FORBID THE ARC
				if(!startingDayStatus_[k-CDMin_+1] || arcCostBestShortSuccCDMin_[s][k][n] >= MAX_COST-1){
					forbidArc( arcsFromSource_[s][k][n][0] );
				}
			}
		}
	}
	...
	for(int s=1; s<pScenario_->nbShiftsType_; s++){
		for(int k=daysMin_-1; k<nDays_; k++){
			for(int n=1; n<=maxvalConsByShift_[s]; n++){
				...
				double c = arcCostBestShortSuccCDMin_[s][k][n];
				updateCost( a , c );
				shortSuccCDMinIdFromArc_.insert(pair<int,int>( arcsFromSource_[s][k][n][0], idBestShortSuccCDMin_[s][k][n]));
			}
		}
...
// Returns true if the given successions contains the given shift
//
bool SubProblemShort::succContainsDayShift(int size, int succId, int startDate, int thatDay, int thatShift){
	return	thatDay >= startDate
			and thatDay < startDate + size
			and allowedShortSuccBySize_[ size ][ succId ][ thatDay-startDate ] == thatShift;
}
...
// // Compute the cost of a single short rotation
// double SubProblemShort::costOfVeryShortRotation(int startDate, vector<int> succ){
...
//
//----------------------------------------------------------------
bool SubProblemShort::solveLongRotationsHeuristic(){
...
// Print the path (arcs, nodes, cost of each arc in the current network, etc.)
//
void SubProblemShort::printPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_spptw_res_cont_Short resource){
...
void SubProblemShort::printShortArcs(){
	for(int s=1; s<pScenario_->nbShiftsType_; s++){
		for(int k=daysMin_-1; k<nDays_; k++){
			for(int n=1; n<=maxvalConsByShift_[s]; n++){
				if(!isArcForbidden(arcsFromSource_[s][k][n][0])){
					int v = principalNetworkNodes_[s][k][n];
					int succId = idBestShortSuccCDMin_[s][k][n];
					vector<int> succ = allowedShortSuccBySize_[CDMin_][succId];
					std::cout << "# " << shortNameNode(v) << " <- (id=" << succId << ") ";
...
void SubProblemShort::testGraph_spprc(){
	...
	vector< vector< boost::graph_traits<Graph>::edge_descriptor> > opt_solutions_spptw_single;
	vector<spp_spptw_res_cont_Short> pareto_opt_rcs_spptw_single;
