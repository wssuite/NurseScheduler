...
bool SubProblem::solveLongRotationsOptimal(){
	...
	vector< vector< boost::graph_traits<Graph>::edge_descriptor> > opt_solutions_spp;
	vector<spp_res_cont> pareto_opt_rcs_spp;

	// ONE SINGLE SINK FOR ALL DAYS
	//
	std::vector<int> initial_label_values(nLabels_);
	if(!(param_.oneSinkNodePerLastDay_)){
		...
		return addRotationsFromPaths(opt_solutions_spp, pareto_opt_rcs_spp);
	}
...
//
bool SubProblem::addRotationsFromPaths(vector< vector< boost::graph_traits<Graph>::edge_descriptor> > paths, vector<spp_res_cont> resources){
	int nFound = 0;
...
//
Rotation SubProblem::rotationFromPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_res_cont resource){
...
void SubProblem::createNodes(){
	...
	//
	sourceNode_ = nNodes_;
	addSingleNode(SOURCE_NODE, {0,0}, {maxRotationLength_, CDMin_});
	...
	for(int k=0; k<nDays_; k++){
		...
		addSingleNode(ROTATION_LENGTH_ENTRANCE, {0,0}, {maxRotationLength_, CDMin_});
		...
		for(int l=0; l<=maxRotationLength_; l++){		// Check nodes: from CD_max (longest free) to maximum rotation length, for each day
			...
			addSingleNode(ROTATION_LENGTH, {0, 0}, {l, max(0, CDMin_-l)});
		}
	...
	//
	sinkNode_ = nNodes_;
	addSingleNode(SINK_NODE, {0,0}, {maxRotationLength_, CDMin_});
...
void SubProblem::addSingleNode(NodeType type, std::vector<int> lbs, std::vector<int> ubs){
...
void SubProblem::addNodeToPrincipalNetwork(int sh, int k, int cons){
	...
	// Create the node
	//
	addSingleNode(PRINCIPAL_NETWORK, {0,0}, {maxRotationLength_, CDMin_});
}
...
void SubProblem::addSingleArc(int o, int d, double baseCost, std::vector<int> consumptions, ArcType type, int shiftID){
  boost::graph_traits< Graph>::edge_descriptor e =
          (add_edge( o, d, Arc_Properties( nArcs_, type, baseCost, consumptions, shiftID ), g_ )).first;
	...
	for (int sh=1; sh<nShiftsType; sh++){
	  unsigned int nShifts = pScenario_->shiftTypeIDToShiftID_[sh].size();
		for(int k=daysMin_-1; k<nDays_-1; k++){
			...
			for(int nCons=1; nCons<maxvalConsByShift_[sh]; nCons ++){
				...
				for (unsigned int s = 0; s < nShifts; s++) {
				  ...
				  addSingleArc(origin, destin, 0, {1,-1}, SHIFT_TO_SAMESHIFT, shiftID);
				}
			}
			...
			for (unsigned int s = 0; s < nShifts; s++) {
			  ...
			  addSingleArc(origin, destin, cost, {1,-1}, REPEATSHIFT, shiftID);
			}
			...
			for(int newSh=1; newSh<nShiftsType; newSh++){
			  if(newSh != sh and ! pScenario_->isForbiddenSuccessorShiftType_ShiftType(newSh,sh)){
			    ...
			    for (unsigned int s = 0; s < nNewShifts; s++) {
			      ...
			      addSingleArc(origin, destin, 0, {1,-1}, SHIFT_TO_NEWSHIFT, newShiftID);
			    }
			  }
			}
			...
			for(int nCons=1; nCons<maxvalConsByShift_[sh]; nCons++){
				...
				addSingleArc(origin, destin, consShiftTypeCost(sh, nCons), {0,0}, SHIFT_TO_ENDSEQUENCE, destin);
			}
		}
		...
		for(int nCons=1; nCons<maxvalConsByShift_[sh]; nCons++){
			...
			addSingleArc(origin, destin, 0, {0,0}, SHIFT_TO_ENDSEQUENCE, destin);
		}
	}
}
...
void SubProblem::createArcsAllRotationSize(){
	...
	for(int sh=1; sh<nShiftsType; sh++){		// For all shifts
		for(int k=daysMin_-1; k<nDays_; k++){				// For all days
			...
			addSingleArc(origin, destin, 0, {0,0}, PRINCIPAL_TO_ROTSIZE, destin);	// Allow to stop rotation that day
		}
	}
	...
	for(int k=daysMin_-1; k<nDays_; k++){
		...
		for(map<int,int>::iterator itRLN = rotLengthNodesForDay.begin(); itRLN != rotLengthNodesForDay.end(); ++itRLN){
			...
			addSingleArc(origin, destin, 0, {0,0}, ROTSIZE_TO_SINK, destin);
		}
		...
		destin = sinkNode_;
		addSingleArc(origin, destin, 0, {0,0}, SINKDAY_TO_SINK, destin);
...
void SubProblem::forbidArc(int a){
	if(!isArcForbidden(a)){
	    Arc_Properties arc_prop = arc(a);
		forbiddenArcs_[a] = arc_prop;
		for(int l=0; l<arc_prop.size(); ++l)
            arc_prop.consumptions[l] = MAX_TIME;
		updateConsumptions(a, arc_prop.consumptions);
	}
}
...
void SubProblem::forbidNode(int v){
	if(!isNodeForbidden(v)){
        Vertex_Properties vert_prop = node(v);
        forbiddenNodes_[v] = vert_prop;
        for(int l=0; l<vert_prop.size(); ++l)
            vert_prop.ubs[l] = vert_prop.lb(l) - 1;
        updateUBs(v, vert_prop.ubs);
	}
}
...
void SubProblem::authorizeArc(int a){
	if(isArcForbidden(a)){
	    auto it = forbiddenArcs_.find(a);
        updateConsumptions(a, it->second.consumptions);
        forbiddenArcs_.erase(it);
    }
}
...
void SubProblem::authorizeNode(int v){
	if(isNodeForbidden(v)){
        auto it = forbiddenNodes_.find(v);
        updateUBs(v, it->second.ubs);
        forbiddenNodes_.erase(it);
    }
}
...
void SubProblem::resetAuthorizations(){
    ...
    for(auto& p: forbiddenArcs_)
        updateConsumptions(p.first, p.second.consumptions);
    forbiddenArcs_.clear();

    for(auto& p: forbiddenNodes_)
        updateUBs(p.first, p.second.ubs);
...
//
void SubProblem::printPath(vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_res_cont resource){
	...
	for( int j = static_cast<int>( path.size() ) - 1; j >= 0;	--j){
		...
		std::cout << "\t\t[" << (isArcForbidden(a) ? "forbidden" : " allowed ") << "]" << std::endl;
	}
	std::cout << "# " << std::endl;
	for( int j = static_cast<int>( path.size() ) - 1; j >= 0;	--j){
		int a = boost::get(&Arc_Properties::num, g_, path[j]);
...
void SubProblem::testGraph_spprc(){
    ...
    vector<spp_res_cont> pareto_opt_rcs_spptw_single;
    ...
    vector<spp_res_cont> pareto_opt_rcs_spptw;
