...
struct SubProblemParam {
  ...
  double spMaxSolvingTimeSeconds_ = LARGE_TIME;
  // ratio for the maximum solving time allowed when computing LB
  double spMaxSolvingTimeRatioForRelaxationAtRoot_ = 15,
         spMaxSolvingTimeRatioForRelaxation_ = 2;
  ...
  double spMinColumnsRatioForIncrease_ = .1;

  // resort nurses at each resolution based on dual values
  bool spSortNursesBasedOnDuals_ = false;
  ...
  // if not computing the optimal relaxation, we try to compute a lower
  // bound when set to true
  bool spComputeLB_ = true;
  ...
  // if true, the solver will stop before trying next execution to solve the
  // other pricing subproblems
  bool rcsspWaitBeforeStartingNextExecution_ = false;
  ...
  // first ignore the lower bounds of every constraint and
  ...
  // true if we apply dssr with several levels: ignore lower bounds, ignore
  // also resources that concern a particular shift or shift type, then
  // add them one by one, then stop ignoring LB
  bool rcsppIncrementalDssr_ = false;
  ...
  // random first source node/start day for the label-setting algorithm
  // heuristic to generate different rotations with different first day
  // stops as soon as rcsppMaxNegativeLabels_ is reached
  // if rcsppMaxNegativeLabels_ is not reached, we have an optimal solution
  bool rcsppRandomStartDay_ = false;

  // when having a max solving time, this parameter is true if all search levels
  // are bounded by a global maximum level
  bool rcsppUseGlobalMaxLevel_ = false;
...
class SolverParam {
 public:
  ...
  // (the difference between two solution costs is at least 1) -> optimal
  ...
  // solve to optimality if enable
  bool solveToOptimality_ = false;

  // if looking for just for an UB, no need to compute the compute the optimal
  // solution at the root node
  bool solveRelaxationToOptimality_ = true;

  // We can thus set a max solving time for all the subproblems
  // for a given iteration
  double maxSolvingTimeRatioForSubproblems_ = .01;
