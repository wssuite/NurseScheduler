...
void RCGraph::printPath(std::vector< boost::graph_traits<Graph>::edge_descriptor > path, spp_res_cont resource){
  ...
  for( int j = static_cast<int>( path.size() ) - 1; j >= 0;	--j){
    ...
    if(origin == source_){                                    //////  SERGEB A MODIFIER
      int firstDay =  boost::get(&Arc_Properties::day, g_, path[j]);
      while(k<firstDay){
        std::cout << " |";
        k++;
      }
    ...
    else if(nodeType( origin ) == PRINCIPAL_NETWORK and k == get( &Arc_Properties::day, g_, path[j]) and
        !get( &Arc_Properties::shifts, g_, path[j]).empty()) {
      std::cout << get( &Arc_Properties::shifts, g_, path[j]).back() << "|";
      k++;
    }
  }
  while(k < nDays_){
