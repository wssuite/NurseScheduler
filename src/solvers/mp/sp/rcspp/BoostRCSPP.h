...
#ifndef NURSESCHEDULER_BOOSTRCSPP_H
#define NURSESCHEDULER_BOOSTRCSPP_H
...
struct spp_res_cont {
    ...
    // the parentDominanceLvl is fixed at the creation and is used for the comparison in the queue.
    // The objective of this level is to ensure that labels of dominated paths are all dominated:
    // the dominant label will be expanded until it reaches the end of the dominated path
    // (before the dominated path will be expanded).
    // Indeed, its parentDominanceLvl will always be higher than the one of the dominated path,
    // and thus will be expanded before.
...
class dominance_spp {
  public:
    ...
    bool operator()(spp_res_cont &res_cont_1, spp_res_cont &res_cont_2) const;
...
class BoostRCSPPSolver : public RCSPPSolver {
  ...
  protected:
    ...
    bool processPath(std::vector<edge> &path, spp_res_cont &rc,
                     std::function<void(spp_res_cont &)> post_process_rc = nullptr, bool printBadPath = true) const;

    RCSolution solution(const std::vector<edge> &path, const spp_res_cont &resource) const;
};

//
// r_c_shortest_path: derived from boost::r_c_shortest_paths_dispatch
//
...
struct rc_spp_visitor {
    rc_spp_visitor(int nPaths, const std::vector<vertex> &sinks,
...
void r_c_shortest_paths_solve(const Graph &g,
  ...
  switch (strategy) {
    ...
    case SP_DOMINANT_FIRST:
      ...
      break;
    default:
      Tools::throwError("Spplabel comparator not defined for the search strategy chosen");
  }
}
...
void r_c_shortest_paths_dispatch(const Graph &g,
  ...
  // definition of the label and the priority queue that stores the labels left to process
  std::priority_queue<Spplabel, std::vector<Spplabel>, Spplabel_Comparator> unprocessed_labels;
  ...
  // build first label
  boost::r_c_shortest_paths_label<Graph, spp_res_cont> *first_label = l_alloc.allocate(1);
  l_alloc.construct(first_label, Label(i_label_num++, rc, nullptr, edge(), s));
  ...
  // vector of iterator: store the last valid position in the list for dominance:
  // every elements before the iterator cannot dominate each other, the ones after haven't been tested for the moment
  typedef std::vector<std::list<Spplabel>::iterator> vec_last_pos_for_dominance_type;
  vec_last_pos_for_dominance_type vec_last_pos_for_dominance(num_vertices(g));
  // store in the map an iterator starting at the beginning of the vector of labels for each vertex
   ...
   * erase it from the list, delete spplabel2 or mark it as dominated, and move it2 to the next label.
   * return true if dominated.
   */
  auto dominance_func = [&dominance, &vis, &l_alloc, &g](Spplabel &spplabel1, Spplabel &spplabel2,
      ...
      if (dominance(spplabel1->cumulated_resource_consumption,
        ...
        if (spplabel2->b_is_processed) {
          ...
          // mark as dominated: will be deleted when popped of the unprocessed list
          // if current_label, it will be deleted latter on.
        else spplabel2->b_is_dominated = true;
        return true;
      }
      return false;
  };
  ...
  /**
    * MAIN LOOP THAT RUNS UNTIL ALL LABELS HAS BEEN PROCESSED OR VIS.ON_ENTER_LOOP RETURNS FALSE
    */
  while (!unprocessed_labels.empty() && vis.on_enter_loop(unprocessed_labels, g)) {
    /**
      * 1 - Pop cur_label from the priority queue unprocessed_labels (choose the one with the higher priority)
      */
    ...
    // object in the respective list<Spplabel> of vec_vertex_labels share their
    ...
    // the function; only the Spplabel object in unprocessed_labels still
    ...
    /**
      ...
      * check with dominance  all the label residing at cur_label->resident_vertex
      */
    assert (cur_label->b_is_valid);
    if (!cur_label->b_is_dominated) {
      ...
      std::list<Spplabel> &list_labels_cur_vertex = vec_vertex_labels[i_cur_resident_vertex];
      auto last_pos_for_dominance = vec_last_pos_for_dominance[i_cur_resident_vertex];

      // if more than just current label at current resident vertex
      // and last_pos_for_dominance is not the last element of the list
      // (do not rechecked labels that were already at the vertex at the previous check)
      // otherwise, go to 2.b
      if (list_labels_cur_vertex.size() > 1
        ...
        //    *: just after the last label checked (will be the first one if first time).
        ...
        // it is set to the end [a list returns the end() for begin() when empty] -> set it to begin()
        if (last_pos_for_dominance == list_labels_cur_vertex.end())
          last_pos_for_dominance = list_labels_cur_vertex.begin();

        // while begin_it hasn't reached the end, compare for dominance with the other labels (end_it)
        while (it1 != list_labels_cur_vertex.end()) {
          ...
          // if it1 is exactly at the last position -> mark the iterator as passed the last position
          if (!it1_at_or_beyond_last_pos_for_dominance && it1 == last_pos_for_dominance)
            it1_at_or_beyond_last_pos_for_dominance = true;

          // initialize second iterator. Will be used to compare labels to begin_iter
          auto it2 = it1;

          // if not first time and it1 before last position -> set it2 to the last position checked
          if (!it1_at_or_beyond_last_pos_for_dominance)
            it2 = last_pos_for_dominance;
          // just move it2 to the next position (one position after last or it1)
          ++it2;
          ...
          while (it2 != list_labels_cur_vertex.end()) {
            ...
            // -> erase it, move to the next position, and continue current loop on it2
            if (dominance_func(spplabel1, spplabel2, it2, list_labels_cur_vertex))
              continue;
              // otherwise move it2 to the next label
            else ++it2;

            // b - if spplabel2 dominates spplabel1
            // -> erase it, move to the next position, and break current loop on it2
            if (dominance_func(spplabel2, spplabel1, it1, list_labels_cur_vertex)) {
              it1_erased = true;
              break;
            }
          }
          // if iterator not erased (i.e., already moved to the next label), move to next label
          if (!it1_erased)
            ++it1;
        } // end while loop for dominance

        // set the iterator at the last element of the list
        vec_last_pos_for_dominance[i_cur_resident_vertex] = --list_labels_cur_vertex.end();
      }
    }
    assert (cur_label->b_is_valid);

    /**
      * 2.b - If cur_label is still non dominated, expand the label on all successors
      */
    if (!cur_label->b_is_dominated) {
      ...
      for (boost::tie(oei, oei_end) = out_edges(cur_vertex, g); oei != oei_end; ++oei) {
        ...
        l_alloc.construct(new_label,
                          Label(i_label_num++, cur_label->cumulated_resource_consumption, cur_label.get(), *oei,
                                target(*oei, g)));

        // populate the label and check its feasibility
        bool b_feasible = ref(g,
  ...
  /**
    * 3 - The main loop has ended, retrieve all the paths that have reached one of the sink in t.
    *     The code has been modified as the boost version implied that all predecessors labels must be non-dominated,
    *     which is not the case it the process is stopped before the end.
    *     Instead, we reconstruct a non-dominated path, based on the non-dominated labels residing
    *     at each certain vertex.
    *     The resulting label at the sink will dominate the one found and could have a different path.
    */
  for (int sink: t) {
    ...
    for (auto csi = dspplabels.begin(); csi != dspplabels.end(); ++csi) {
      ...
      while (p_cur_label->num != 0) {
        ...
        for (Spplabel label: vec_vertex_labels[cur_vertex]) {
          spp_res_cont pred_cont = label->cumulated_resource_consumption, new_cont = pred_cont;
          // d - expand label: if feasible, check if new label dominate the current one
        ...
        //
        // The main reason a label could not be found is that the predecessor label has been dominated and deleted,
        // and the label residing at the current vertex could not be expanded in a feasible way.
        // That could happen if the LB of a label is hard
        if (!found_label) {
          // if the LBs are hard, discard the path
          if (get(&Vertex_Properties::hard_lbs_, g, p_cur_label->resident_vertex)) {
            p_cur_label = nullptr;
            break;
          } else
            Tools::throwError("Predecessor label could not be found when reconstructing solution path in the RCGraph.");
        }
        assert(p_cur_label->b_is_valid);
        assert(cur_vertex == p_cur_label->resident_vertex);
      }
      if (!p_cur_label)
        continue;
      opt_solutions_spp.push_back(path);

      // 2 - rebuild label (current label should be source)
      spp_res_cont cur_cont = p_cur_label->cumulated_resource_consumption;
      for (int i = path.size() - 1; i >= 0; --i) {
        spp_res_cont old_cont = cur_cont;
        ref(g, cur_cont, old_cont, path[i]);
      }
      assert(dominance(cur_cont, original_cont));
  ...
  */
  ...
  BGL_FORALL_VERTICES_T(i, g, Graph) {
      const std::list<Spplabel> &list_labels_cur_vertex = vec_vertex_labels[i];
      for (auto csi = list_labels_cur_vertex.begin(); csi != list_labels_cur_vertex.end(); ++csi) {
...
#endif //NURSESCHEDULER_BOOSTRCSPP_H
