...
class dominance_spp {
 public:
  dominance_spp(const std::vector<bool> &labelOrder,
                const std::vector<int> &labelsMinLevel,
                double epsilon) :
      labelsOrder_(labelOrder),
 ...
 private:
  ...
  // if label value is below (or above based on order) this level, label
  // cannot be dominated. this is necessary as labels pricing could be even 0
  // for dominated label and label could be reseted
...
class BoostRCSPPSolver : public RCSPPSolver {
 public:
  ...
  std::vector<RCSolution> solve(int nLabels,
                                const std::vector<int> &labelsMinLevel,
...
void r_c_shortest_paths_dispatch(const Graph &g,
  ...
  while (!unprocessed_labels.empty()
    ...
    if (!cur_label->b_is_dominated) {
      ...
      for (boost::tie(oei, oei_end) = out_edges(cur_vertex, g); oei != oei_end;
           ++oei) {
      ...
      while (p_cur_label->num != 0) {
        ...
        for (Spplabel label : vec_vertex_labels[cur_vertex]) {
          spp_res_cont pred_cont = label->cumulated_resource_consumption,
              new_cont = pred_cont;
          // d - expand label: if feasible, check if new label dominate the
          // current one
        ...
        if (!found_label) {
          ...
          if (get(&Vertex_Properties::hard_lbs_,
                  g,
                  p_cur_label->resident_vertex)) {
            p_cur_label = nullptr;
            break;
          } else {
