...
std::string GlobalStats::toString() {
  ...
  statStream << timeInitialSol_ << "\t" << timeImproveSol_ << "\t"
             << timeInitialSol_ + timeImproveSol_ << "\t";
...
void GlobalStats::add(const GlobalStats &stats) {
  ...
  if (!stats.nbImprovementsWithNursesSelection_.empty()) {
    if (nbImprovementsWithNursesSelection_.empty()) {
      nbImprovementsWithNursesSelection_.insert(
          nbImprovementsWithNursesSelection_.begin(),
          stats.nbImprovementsWithNursesSelection_.size(),
          0);
    }
    if (nbImprovementsWithDaysSelection_.empty()) {
      nbImprovementsWithDaysSelection_.insert(
          nbImprovementsWithDaysSelection_.begin(),
          stats.nbImprovementsWithDaysSelection_.size(),
          0);
    }
    if (nbImprovementsWithRepair_.empty()) {
      nbImprovementsWithRepair_.insert(nbImprovementsWithRepair_.begin(),
                                       stats.nbImprovementsWithRepair_.size(),
                                       0);
    }
    for (unsigned int i = 0;
         i < stats.nbImprovementsWithNursesSelection_.size(); i++) {
      nbImprovementsWithNursesSelection_[i] +=
          stats.nbImprovementsWithNursesSelection_[i];
    }
    for (unsigned int i = 0; i < stats.nbImprovementsWithDaysSelection_.size();
         i++) {
      nbImprovementsWithDaysSelection_[i] +=
          stats.nbImprovementsWithDaysSelection_[i];
    }
